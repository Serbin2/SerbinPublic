...
RSQNote::RSQNote() : m_time(0), m_Key(0), m_Type(0), m_PrevKey(0)
#ifndef RSTOOL
...
RSQNote::RSQNote(float time, int key) : m_time(time), m_Key(key), m_Type(0), m_PrevKey(0)
#ifndef RSTOOL
, m_Distance(D2D1::Point2F(0, 0)), m_Dst(D2D1::Point2F(0, 0)), m_Src(D2D1::Point2F(0, 0))
#endif
{
...
#ifndef RSTOOL
int RSQNote::Judge(float time) // 곡이 재생되는 시간을 기준으로 키가 입력된 시간을 받아야 합니다.
{
	float judTime = m_time - time; // 양수일 경우 Early, 음수일 경우 Late
...
{
	SetPositionW(Vector2(m_Dst.x - 125, m_Dst.y - 125));
	Identify();
	...
	MakeTransformMatrix();
	D2DGraphics::GetInstance().SetTransform(TransformMatrix);
	D2DGraphics::GetInstance().DrawImage(ResourceManager::GetInstance().GetImageSheet(ResourceID::PLAY_NOTE_CIRCLE), 0, 0);
	Identify();
}
